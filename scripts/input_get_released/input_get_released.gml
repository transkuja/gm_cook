function check_key_released(_device, _input_key) {
	var KeyGetter = keyboard_check_released(_input_key);
	if ( _input_key == mb_left or _input_key == mb_right or _input_key == mb_middle or
		_input_key == mb_any or _input_key == mb_none ){
				KeyGetter = device_mouse_check_button_released(_device, _input_key);
		}
		
	return KeyGetter;
}

function check_pad_released(_device, _input_pad) {
	var PadGetter = gamepad_button_check_released(_device, _input_pad);
	switch(_input_pad){
		...
		case gp_axisR_up:
			...
			PadGetter=0;
			if ( !__rut && __rutp ){ PadGetter = 1; __rutp = __rut; }
		break;
		#endregion 
		...
		case gp_axisR_down:
			...
			PadGetter=0;
			if ( !__rdt && __rdtp ){ PadGetter = 1; __rdtp=__rdt; }
		break;
		#endregion
		...
		case gp_axisR_left:
			...
			PadGetter=0;
			if ( !__rlt && __rltp ){ PadGetter = 1; __rltp=__rlt; }
		break;
		#endregion
		...
		case gp_axisR_right:
			...
			PadGetter=0;
			if ( !__rrt && __rrtp ){ PadGetter = 1; __rrtp=__rrt; }
		break;
		#endregion
		...
		case gp_axisL_up:
			...
			PadGetter=0;
			if ( !__lut && __lutp ){ PadGetter = 1; __lutp=__lut; }
		break;
		#endregion
		...
		case gp_axisL_down:
			...
			PadGetter=0;
			if ( !__ldt && __ldtp ){ PadGetter = 1; __ldtp=__ldt; }
		break;
		#endregion
		...
		case gp_axisL_left:
			...
			PadGetter=0;
			if ( !__llt && __lltp ){ PadGetter = 1; __lltp=__llt; }
		break;
		#endregion
		...
		case gp_axisL_right:
			...
			PadGetter=0;
			if ( !__lrt && __lrtp ){ PadGetter = 1; __lrtp=__lrt; }
		break;
		#endregion
...
function input_get_released(_device, _inputName){
	...
	var KeyGetter = check_key_released(_device, _INPUT_KEY);
	if (!KeyGetter && _EXTRA_KEYS != []) {	
		for (var _i = 0; _i < array_length(_EXTRA_KEYS); _i++) {
			if (check_key_released(_device, _EXTRA_KEYS[_i])) {
				KeyGetter = true;
				break;
			}
		}
	...
	var PadGetter = check_pad_released(_device, _INPUT_PAD);
	if (!PadGetter && _EXTRA_PAD != []) {	
		for (var _i = 0; _i < array_length(_EXTRA_PAD); _i++) {
			if (check_pad_released(_device, _EXTRA_PAD[_i])) {
				PadGetter = true;
				break;
			}
		}
