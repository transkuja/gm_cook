function json_buffer_write_slice(buffer, data_buffer, data_start, data_end){
	var start = data_start;
	var _next = data_end - start;
	if (_next <= 0) exit;
	var buf = buffer;
	...
	buffer_copy(data, start, _next, buf, pos);
	buffer_seek(buf, buffer_seek_relative, _next);
}
...
function json_beautify(json_string) {
	...
	var _next; // number of bytes to be copied
	var need;
	var nest = 0;
	while (pos < size) {
	    ...
	    switch (c) {
	        case 9: case 10: case 13: case 32: // `\t\n\r `
	            json_buffer_write_slice(rb, fb, start, pos - 1);
	        ...
	        case 34: // `"`
	            ...
	            break;
	        case ord("["): case ord("{"):
	            json_buffer_write_slice(rb, fb, start, pos);
	            // skip over trailing whitespace:
	            while (pos < size) {
	                switch (buffer_peek(fb, pos, buffer_u8)) {
	                    case 9: case 10: case 13: case 32: pos += 1; continue;
	        ...
	        default:
	            if (c >= ord("0") && c <= ord("9")) { // `0`..`9`
	                ...
	                if (till < pos) { // flush if number can be shortened
	                    json_buffer_write_slice(rb, fb, start, till + 1);
	                    start = pos;
	                }
	            }
	    }
